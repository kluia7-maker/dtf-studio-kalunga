...
function openVitrineOrder(campId, itemIdx){
  ...
  document.getElementById('vtOrderItem').innerHTML=`
    <div style="display:flex;gap:12px;align-items:center;background:var(--surface2);border-radius:12px;padding:12px">
      ${item.imgUrl?`<img src="${item.imgUrl}" style="width:70px;height:70px;object-fit:cover;border-radius:8px;background:#111">`:'<div style="width:70px;height:70px;border-radius:8px;background:var(--surface3);display:flex;align-items:center;justify-content:center;font-size:32px">🖼️</div>'}
      <div>
        <div style="font-size:13px;font-weight:600;margin-bottom:2px">${item.nome||''}</div>
  ...
  const tams = (item.tamanhos||[]).length>0 ? item.tamanhos : ['P','M','G','GG','XGG'];
  document.getElementById('vtSizeBtns').innerHTML = tams.map(s=>
